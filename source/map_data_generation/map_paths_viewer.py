...
def _resolve_cfg_path(config_path: Path, p: str | None) -> str | None:
    if not p:
        return None
    pp = Path(p).expanduser()
    if pp.is_absolute():
        return str(pp)
    # 相对路径按 config.json 所在目录解析，避免从仓库根目录运行时找不到文件
    return str((config_path.parent / pp).resolve())
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help="可选：配置文件（json），参考本目录 config.json，包含 routes_path/points_path/map_path",
    )
    parser.add_argument(
        "--paths",
        required=False,
    ...
    routes_path = _resolve_cfg_path(config_path, routes_path)
    if not routes_path:
        raise ValueError("routes_path 为空。")
    paths_file = Path(routes_path)
    ...
    if args.map:
        ...
        mp = _resolve_cfg_path(config_path, cfg["map_path"])
    ...
    if args.points:
        ...
        pp = _resolve_cfg_path(config_path, cfg["points_path"])
        points_candidate = Path(pp) if pp else None
    else:
