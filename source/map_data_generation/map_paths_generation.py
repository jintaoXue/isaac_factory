...
示例用法：
  python3 source/map_data_generation/map_paths_generation.py \\
      --map source/map_data_generation/occupancy_map4human.png \\
      --points source/map_data_generation/map_points_human.json \\
      --out source/map_data_generation/map_routes_human.json
"""
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="可选：config.json（建议用本目录，含 png_image_coordinates / isaac_sim_coordinates），"
    ...
    parser.add_argument(
        ...
        required=True,
        type=str,
        help="占据地图图片路径，例如 source/map_data_generation/occupancy_map4human.png",
    )
    parser.add_argument(
        "--points",
        required=True,
        type=str,
        help="路网点 JSON 文件路径，例如 source/map_data_generation/map_points_human.json",
    )
    parser.add_argument(
        "--out",
        required=True,
        type=str,
        help="输出最短路 JSON 文件路径，例如 source/map_data_generation/map_routes_human.json",
    )
    parser.add_argument(
        "--obstacle-threshold",
        type=float,
