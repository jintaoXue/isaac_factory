...
def _build_argparser() -> argparse.ArgumentParser:
    ...
    p.add_argument(
        ...
        default=None,
        type=str,
        help="可选：config.json（建议用本目录）；若含 png/isaac 角点则保存为 Isaac Sim 坐标并写入内嵌角点",
    )
    p.add_argument(
        "--map",
        required=True,
        type=str,
        help="占据栅格地图图片路径（png/jpg等），例如 source/map_data_generation/occupancy_map4robot.png",
    )
    p.add_argument(
        "--out",
        required=True,
        type=str,
        help="输出路网点文件（.json 或 .csv），例如 source/map_data_generation/map_points_human.json",
    )
    p.add_argument(
        "--load",
        default=None,
    ...
    p.add_argument(
        ...
        default=None,
        type=str,
        help="可选：导出“带点的地图图片”时使用的底图（例如 source/map_data_generation/occupancy_map4human.png）",
    )
    p.add_argument(
        "--overlay-points",
        default=None,
    ...
    p.add_argument(
        ...
        default=None,
        type=str,
        help="可选：导出叠加图的输出图片路径（例如 source/map_data_generation/map_with_points_human.png）",
    )
