"""Run start<=5/10/15 Min8 curriculum on dense_i1 plus 2/3/4-dim mix."""
...
def main() -> int:
    ...
    for max_start in (5, 10, 15):
        name = (
            f"dense_i1_mix_start{max_start}_min8_cause4_opt_"
        ...
        cmd = [
            ...
            "factory_bn.train",
            "--config",
            f"factory_bn/configs/FactoryBN_dense_mix_start{max_start}_min8_opt.json",
            "--data_dir",
            "raw_data/dense_i1_mix",
            "--save_dir",
            str(save_rel),
            "--device",
            "cuda",
        ...
        print(f"[mix start<={max_start} Min8]", " ".join(cmd), flush=True)
