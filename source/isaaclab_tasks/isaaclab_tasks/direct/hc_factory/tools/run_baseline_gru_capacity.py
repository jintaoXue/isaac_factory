#!/usr/bin/env python3
"""Run the registered four GRU-width controls, then frozen diagnostics.
...
from preflight_baseline_gru_capacity import sha
...
def main():
    ...
    preflight_path = d / "baseline_gru_capacity_preflight20260913.json"
    assert sha(preflight_path) == args.preflight_sha256
    pf = json.loads(preflight_path.read_text())
    assert pf["status"] == "four_gru_capacity_real_208_preflight_cases_passed"
    ...
    final = d / "baseline_dense_gru_capacity_metrics_20260913.json"
    ...
    def paths(model, seed):
        ...
        snap = d / f"baseline_gru_capacity_{model.lower()}s{seed}_training20260913.json"
        return out, snap
    ...
    if args.phase == "all":
        ...
        for model, seed in CASES:
            ...
            print("START_GRU_CAPACITY_TRAIN", model, seed, flush=True)
            subprocess.run([sys.executable, "-B", "-u", str(tools / "train_dense_baseline_control.py"),
                ...
                "--variant", "gru_capacity32", "--archive_tag", TAG, "--device", "cuda:0"], check=True)
            guard()
            ...
            assert control["variant"] == "gru_capacity32" and not control["test_evaluated"]
            ...
            print("GRU_CAPACITY_TRAIN_COMPLETE", model, seed, "best", summary["best_epoch"], "epochs", summary["epochs_trained"],
                  "UP", round(sr["report_recall_upcoming"] * sr["n_true_upcoming"]), "F1", sr["report_f1"], "snapshot", sha(snap), flush=True)
    ...
    for model, seed in CASES:
        ...
        for checkpoint, split in (("best", "validation"), ("best", "train"), ("last", "validation"), ("last", "train")):
            ...
            path = d / f"baseline_gru_capacity_{model.lower()}s{seed}_{checkpoint}_{split}20260913.json"
            if not path.exists():
                print("START_GRU_CAPACITY_DIAG", model, seed, checkpoint, split, flush=True)
            ...
            print("GRU_CAPACITY_DIAG_VERIFIED", model, seed, checkpoint, split,
                  "AP", result["ranking"]["upcoming_vs_negative"]["tie_aware_average_precision"],
                  "UP", round(canonical["report_recall_upcoming"] * canonical["n_true_upcoming"]), flush=True)
    ...
    print("GRU_CAPACITY_BATCH_COMPLETE", final.stat().st_size, sha(final), flush=True)
