...
对齐最新 dev_tyx 评估。当前完成本地 v6 指标实现及只读 raw 审计，服务器重建和训练
尚未执行。第 24 节记录新进度；前 23 节是旧 v5 开发结果，不能与新 dense 指标混排。
...
保留 split、CSV/内存导出与基线张量对齐测试。真实服务器数据包尚未重建，尚无新 v6 训练结果。
远程操作工具报告本机锁屏，需用户手动解锁才能继续 UU 终端操作；这不是训练已启动后在等待。
下一次实际执行应先复查服务器分支、空闲 GPU、Python 环境和上述既有模型目录，再部署运行。
