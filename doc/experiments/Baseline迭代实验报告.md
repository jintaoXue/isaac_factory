...
工作状态（2026-09-11）：用户已授权开启新一轮高质量数据扩充及 B4/B5 优化，并要求
对齐最新 dev_tyx 评估。服务器已完成 v6 数据重建、全样本对齐检查及四次 B4/B5 控制
训练。下一轮搜索尚未启动，优先确认主实验实际数据包与 split。第 24 节记录新进度；
前 23 节是旧 v5 开发结果，不能与新 dense 指标混排。第 25 节按共同评估规则并列记录
当前 B4/B5 与最新 BNPDFormer 文档结果，明确区分数据包和结果来源。
正式冻结仍需独立评估及主实验对照一致性审计，不把阶段收尾解释为证明方法上限。
...
当前已完成 B4 seed42/43 和 B5 seed42，B5 seed43 的 Python PID3978066 实查存活。
前三次已选 checkpoint 的 upcoming recall 为 0、0.0138、0.0138，不能把中途 0.0414
作为最终结果；全部结束后再作完整判断。已有 v5 加权/抽样结果提示不可无边界重复
此方向，本次仅检验新 dense 标签和数据下的一个已登记权重，不扩展成多倍率搜索。
...
提交前的二分类头做 10 组精确参数/前向/反向对照均一致。三分类候选尚未部署或训练，
其分数不能填入任何结果表。目标尚未达到，后续必须基于完整权重对照与独立诊断继续。
