...
### 47.7 四训练、16候选诊断及16父缓存配对全部完成并独立核验

暂停期间原批正常结束，恢复后只读核验训练pane783323和配对pane840337均dead/exit0，没有重启或重复推理。固定源码182ab93自动完成缓存配对；新增独立核验源码9eb41da85eef1e23a45f7039478a5480028d5c0f只fetch对象执行，逐项核对四训练快照/实际配置/完整history/正式指标，48当前文件、44旧归档成员、4份near父归档、16候选与16父JSON、报告命中减起点漏报语义、14运行源码和6数据stat，全部通过。runtime仍dev_xwt@90a41a5且tracked clean，主仓库未改、test未用。

| 模型 / seed | best / total | 正式P / R / F1 | upcoming：父near→GRU32（各145） |
|---|---:|---|---:|
| B4 / 42 | 6 / 16 | .838052 / .675799 / .748231 | 2→0 |
| B4 / 43 | 6 / 16 | .800208 / .702283 / .748054 | 2→0 |
| B5 / 42 | 4 / 24 | .847302 / .673973 / .750763 | 1→0 |
| B5 / 43 | 27 / 47 | .836084 / .694064 / .758483 | 2→3 |

单个B5 seed增加一个命中，另一个下降；B4两seed都下降，没有稳定可采用改善。继续保留原baseline对照，不把当前松散GRU32负消融权重自动当作已采用最优模型。

| 模型 / seed | 末轮epoch：父→候选 | train AP：父→候选 | validation AP：父→候选 |
|---|---:|---:|---:|
| B4 / 42 | 12→16 | .08213004→.03286623 | .00954082→.00849409 |
| B4 / 43 | 15→16 | .17993384→.02433937 | .00947917→.00991731 |
| B5 / 42 | 26→24 | .30190383→.03799636 | .00912045→.00930803 |
| B5 / 43 | 25→47 | .30942241→.05748819 | .00863192→.00898416 |

候选末轮upcoming命中train分别45/13/49/52（各595），validation0/1/3/2（各145）；末轮仅诊断，不替换上表正式best。AP不依赖报告阈值，但两模型的实际早停轮次并未固定相同，不能声称这是相同optimizer更新数下容量的纯因果效应。三组训练更久而拟合更低，B5 seed42略少两轮；整体仍支持该容量压缩方案降低训练拟合、没有改善未见排序的描述。报告阈值由各自原协议选取，不能把不同模型的概率绝对值直接当作校准一致。

这项负结果不否定过拟合证据，不证明架构上限，也不能单独证明独立数据不足。它限制的是具体GRU128→32并伴随宽度依赖读出/头缩小的方案，不能自动扩展为所有正则化或所有架构都无效。禁止盲目扫描相邻宽度、重训父模型或重复16诊断/配对。

- baseline_dense_gru_capacity_metrics_20260913.json：2379459 bytes，SHA 5fe983e0b0e0031afcaf58b92bc85d12a750bf4ce4cb5a67845f81f0bbc15011。
- baseline_gru_capacity_parent_comparison20260913.json：27620 bytes，SHA f1e7ed4904a03ad38d2dd44ed8f558e94e03e074894c83fd4c1f407c3be2e791。
- baseline_gru_capacity_comparison_wait_complete20260913.json：540 bytes，SHA 08c40413abde5fea1a232cf892f1586694f25bf7f904e6c2bbbb3c9eb918e3b2。
- baseline_gru_capacity_final_verification20260913.json：36732 bytes，SHA c600344f63566090541ae8c788f2055d48eb0ffc64aa2663a553175fcbdffbb6。

B5 seed43训练快照SHA 6f776fb0e6ead4e776738a9502d78afbbab2c549a35b992a3bef58cd949ff4e9；完整P/R/F1=.8360836083608361/.6940639269406392/.7584830339321357。4组训练及全部已登记分析均完成，不留待重复任务。具体低召回根因和稳定改善目标仍未完成。
...
## 49. 固定208内的数据量诊断：54对107个训练episode

### 49.1 先登记标识分组及等更新预算，尚未预检或训练

第47.7节容量控制整批完成，缩小网络没有稳定验证收益。第46节证明107-fit相对两个未见集存在严重泛化差距，但未直接干预独立训练episode数量。本节检查54→107个episode在固定评估集、固定网络和相同optimizer更新预算下是否改善upcoming排序。代码/报告去重搜索未见本范围已有data_fraction/episode_fraction/学习曲线控制；旧134与当前208之间的不同数据/协议结果不能替代这项嵌套比较。不重复107-fit训练，不追加新episode。

固定数据仍是原208、138/30/40分割。只在已冻结的107-fit内按raw run分层取ceil(107/2)=54个episode。每run先floor(n/2)，剩余额度按固定salt的run标识hash分配给奇数run，run内episode按同salt的标识hash取样；所有21个run两半均有代表，保留54、余下53。salt=baseline_data_quantity54_20260913_v1，选择函数只读取sample_index/group_id/split，标签和预测完全不参与。两份原未见集31-heldout与30-original_validation逐样本保持不变，不使用test。余下53不得参与训练、归一化拟合或选择。

54-fit归一化须重新拟合并在内存重建near23；预处理分组fit54/heldout84（原31+余53）/原validation30只是为了覆盖完整原train/val，不代表将84误当原31评估集。神经评估仍固定fit54、原31-heldout、原30-validation。父107-fit的训练预测对同54-fit目标通过旧NPZ筛选复用，原31/30预测直接复用；禁止父forward或重训。

模型为原near B4/B5、GRU128、seed42各一组，沿用第46节损失/优化器/学习率日程，固定.70，保留10/30/60检查点，不早停、不挑阈值。为避免数据少同时更新次数少的混淆，定义60个参考训练区间，每区间与父相同18095个样本呈现；仅从54-fit窗口均匀打乱循环取样，截断到18095。保持B4每区间754次更新（末batch23），B5每区间1131次（末batch15），总45240/67860。增加重复次数不是增加独立数据，报告必须同时写54/107与真实呈现次数。父训练已经是107窗口全量shuffle；新循环采样和54-fit归一化属于数据量干预流程，不能把结果扩展为任意采样/数据生成方案的结论。

预先指定主要描述为第60参考区间两固定未见集的upcoming AP，三检查点全部报告。如果两模型在两个未见集均随54→107增加而改善，支持此数据量范围内增加训练episode有益；若都停留低值或方向不一致，则没有获得这段范围的数据量收益证据，仍不能证明数据已经足够。AP/召回同时保留；不依据结果重选54或扩大组数。同一split/seed的一次嵌套比较不能估计跨新run/seed稳健性，也不能外推超过107的数据收益或宣称必然达到.5。

新增prepare_baseline_data_quantity.py只负责冻结标识计划，不读标签、不forward、不训练；本地4检查通过：精确54且21 run均覆盖、标签/迭代顺序不影响选择、归一化与评估分组区别、重复/缺失/跨split/test拒绝。初次本地测试文件路径误放至新tests目录，已移回原source/isaaclab_tasks/test，并仅移除本轮误建空目录；旧目录及服务器未动，修正路径后4检查再次通过。

当前服务器尚未执行计划；数量诊断的等更新训练driver/真实预检尚未实现，不得拿旧107-fit driver直接运行新计划或更改旧产物。计划将保存D/baseline_data_quantity54_plan20260913.json；原capacity和第46/48所有已登记工作已完成。原目标保持未达到。


### 49.2 服务器标识计划已冻结；采样核验终态待回读，尚未训练

固定源码8e6d26f997e5c8c34b376fa345ea6da8cc74611b只fetch对象，4项服务器检查通过后生成一次不可重选的54-fit标识计划。fit54=9050窗口，余下53=9045窗口；原heldout31=5764、原validation30=5439不变，21 raw run均覆盖。没有读取标签或预测用于选择，没有模型forward/训练。runtime仍90a41a5，运行源码/原数据stat及48正式路径当前模型文件保护通过。

- baseline_data_quantity54_plan_tests20260913.json：933 bytes，SHA 1b564c9324e77acaf4439271aba0cd139fcd113374f631019d4ae8986a60c1e6。
- baseline_data_quantity54_plan20260913.json：1176014 bytes，SHA 8950601ac88e2a39c57fe2720f8eec60e314d6a801a95a03041b4ada5a1c532a。

另起独立核验命令，重算全部episode配额、hash选择和样本归属，并检查实际服务器RandomSampler的循环呈现与更新预算。首次调用conda Python时使用-I屏蔽了Isaac训练依赖路径，报ModuleNotFoundError:torch，未写出独立核验文件、未开训。随后仅修正该核验的环境：恢复原tmux PYTHONPATH，删除PYTHONHOME，torch子进程使用-B而非-I，已提交执行；用户要求本轮收尾时尚未成功回读结束输出，所以不能登记为通过。

接续先只读D/baseline_data_quantity54_plan_verification20260913.json是否已生成并核验内容/来源，或检查原命令终态；不能直接再次运行写文件命令，更不能重选计划/重跑4测试。神经训练driver、拟合54归一化与真实CUDA预检仍未完成；没有数量诊断新权重或结果。


## 50. 用户明确的baseline架构边界：组件比较不等于移植主模型设计

用户再次明确：不能为了提高召回强行改变B4/B5架构，否则失去对主实验的比较价值。后续保持已登记B4 GCN+GRU、B5 GAT+GRU及共同任务适配，不把BNPDFormer专有历史状态嵌入、传播、prefix等设计追加进baseline。历史负消融仅作为诊断证据保留，不因为文件当前存在或松散权重被覆盖就成为正式采用的baseline。新第49节数据量诊断保持原near/GRU128结构与参数配置；只干预用于拟合的episode数量并对齐更新预算。

只读现有model.py证据：第637–638行创建并零初始化nn.Embedding(8,embed_dim)；_cluster_fused在第756–764行读取hist_cluster并将其可学习向量加到h_last；事件分支第909行使用该融合表示。相对于当前B4/B5，这确实是BNPDFormer实现中的额外状态编码路径。不能由这些代码证明该设计在文献中独有，也不能把它描述成所有Transformer固有且baseline必须补齐的组件。

第34.3/43.1节已有证据：该历史状态由已观测操作特征产生，baseline27维历史X已经包含所需队列、等待、活跃、阻塞、饥饿等信号。29,298个train/validation样本上的相关状态核验一致，没有发现这项离散状态因格式或归一化导致的信息缺失。公平共同输入应对齐可观测信号、时间范围、标签与评估协议；并不要求各模型拥有相同的可学习编码模块。增加该嵌入将得到“B4/B5加主模型状态编码”的新变体，不能把成绩混入原baseline。

当前只能把这个组件记为建模方式差异，不能列为低upcoming根因。第41节已记录主prefix8 .633对应的完整权重/源码/原始配置/共同包仍缺；现版含模块不证明历史高分用了它，更不能证明收益来自它。要做组件因果归因，需要匹配实验及主模型自身的受控消融；当前不改主仓库、不启动主模型训练，也不将缺包当理由再次重复已完成搜索。

因此当前结论应写成“B4/B5在已完成配置下存在明显跨episode泛化失败；具体数据覆盖或表示学习机制仍未锁定”，不能写“已证明少了主状态编码所以召回低”，也不能写“加上该组件才能算公平”。现阶段继续第49节数据量/训练诊断；任何改变这一架构边界的后续工作都不在当前默认优化范围内。


## 51. 本轮收尾：已确认泛化失败，准确根因和稳定改善尚未找到

用户要求先收尾总结，不再启动新训练，后续明确接续后再推进。原目标没有达成；收尾不表示已找到架构限制、数据不足的唯一原因或可采用提升。

1. 最新GRU容量控制四组正式命中0/0/0/3（各145），与父near2/2/1/2相比无稳定收益；整批训练、16诊断、缓存配对及独立核验全部结束。第46节真实神经留出、第48节全厂已记录扰动分组也全部完成，均不可重跑。
2. 真正训练排除的episode同样弱：第46节固定60轮B4 fit402/451、heldout7/144、原val7/145；B5 fit274/451、heldout7/144、原val4/145。训练拟合能力与跨episode泛化能力明显分离。这里为107-fit诊断，不能和正式138-train的best分数混用。
3. 已有证据表明“原validation特别难”“缺少同类别正例支持”“起点匹配错误”“仅全局概率阈值尺度”“仅四类已记录未来新启动”均不足以单独解释差距。各项结论限于相应核验范围，不扩大为完全排除数据分布、全部外部机制或所有校准方法。
4. 数据独立性/覆盖不足、表示依赖episode偶然关联、可迁移前兆较弱仍未区分。训练能拟合不证明学到了通用规律，也不排除架构归纳偏好；GRU128→32负结果只否定该具体优化方案的稳定收益，不能声称维度/架构已完全排除。
