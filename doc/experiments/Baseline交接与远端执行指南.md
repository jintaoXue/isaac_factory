...
更新时间：2026-09-15。先读本文件，不需要把旧对话全部装入上下文。
...
十二组均完成，不重训、不重新收尾；B2 Start5恢复时没有重新拟合，B4/B5保持原结果。本次只完成本地报告和索引更新，没有新服务器命令、UU操作、训练或诊断，test未使用。主模型当前同口径结果尚未汇入，新增根因实验继续暂停。

**以下保留历史时点记录；其中“当前/待执行/待导出/尚未完成”不覆盖本段终态。**
...
**最新用户请求：仅审计 dev_tyx 更新。**已 fetch 到 `7b2ab393e838c1b54a78b2f125d41e0066d53ce0`，没有合并或启动训练，也没有访问服务器。本次发现新三档 Min8 使用 20 格标签、opt 改用 will15 选模且课程式热启；20 格不能生成起点索引 13–15 且持续 8 格的正例。新文档披露旧 204 包有跨 split 重复，clean 入口引用配置未提交，需先冻结共同数据和协议。完整证据及待对齐清单见 [dev_tyx_7b2ab39对齐审计.md](dev_tyx_7b2ab39对齐审计.md)。保留固定 208、旧结果和第 50 节架构边界；本次请求不自动恢复第 49 节数量训练。
...
**最新只读发现（48.2–48.3）：**全工厂四类已记录runtime START关联已完成，4服务器检查与740目标独立核验通过。仅同资源/仅其他资源/无全厂新启动：fit221/51/179、heldout78/8/58、原validation80/22/43（both均0）。旧无局部组65包含22个其他资源启动，不能当全无未来事件；无新启动组也不排除quality hold等机制。已登记复用第46节18缓存分组，4本地检查通过，服务器尚未执行；不forward，不混为GRU32成绩。容量原pane783323仍live，B4两seed均训练完成0/145，B5 seed42已观察18轮，checkout仍固定90a41a5。

**新增只读检查登记（48.1）：**补齐既有“同资源未来启动”关联到四类已记录runtime事件的全工厂范围，740个既有train/validation目标、固定半开边界，4本地测试通过，服务器尚未执行。无checkpoint/forward/训练/未来输入；不把时间伴随当因果或不可预测性。原GRU32容量批次783323继续、checkout固定90a41a5。最新训练终态以47.5记录及实时进程为准。

**最新进度（47.5）：**GRU32首组B4 seed42已完成并独立核验，best6/total16，P/R/F1=.83805/.67580/.74823，upcoming0/145（父near2/145），暂未改善。B4 seed43已到epoch4，原pane783323仍live，B5两组和16候选诊断由原driver接续。运行checkout固定90a41a5，禁止pull/重训首组/重复诊断。配对源码182ab93已fetch，3服务器检查与父16缓存兼容检查通过；整批完成后按实际候选SHA运行一次纯缓存配对，父不重训。原目标未完成。

**最新执行状态（47.4）：**gru_capacity32已真实开训，服务器dev_xwt固定90a41a5，pane783323/实际Python783366，B4 seed42已核验epoch12。3服务器测试、四组CUDA预检及独立48旧文件/6数据stat/14源码核验通过；首组旧11训练文件已验证归档，其他3组各12文件不变。按原协议继续四训练+16诊断，期间禁止pull或重启原批。日志grucapacity20260913_train.log，最终baseline_dense_gru_capacity_metrics_20260913.json待完成。纯缓存配对脚本3本地检查通过，服务器检查待做；父16诊断复用。前12轮尚无upcoming提升，完整结论待最终结果。

**当前最新状态（§47，旧阶段为历史）：**容量去重盘点完成，当前208的50份配置、其他manifest56份均GRU128。登记唯一gru_capacity32（tag grucapacity20260913）：共同near父对照，GRU128→32且原宽度依赖读出/头随之缩小；原138/30/40分割、两模型两seed、原max60/早停/阈值流程。B4参数273054→31230、B5 285982→44158。3本地检查通过，预检/driver已实现；尚未部署/真实预检/训练。必须先核验服务器旧任务已结束再FF更新，真实CUDA预检通过及独立验证后启动，复用原目录与父诊断，不重跑旧搜索。详见§47及JSON最新key。

**当前最新状态（46.14，以下旧阶段为历史）：**真正神经episode留出整批已完成并独立核验：B4/B5各60轮、18视图、512次episode统计、两模型支持分组与校准界，无待完成该批诊断。B4末轮fit AP=.93113/召回402/451，heldout7/144、原validation7/145；B5 fit AP=.65787/274/451，heldout7/144、原validation4/145。两骨干均跨episode泛化失败，有联合支持者也弱；18视图起点漏报均0，who与report计数实际一致。P≥.80时全局阈值/单调校准无法把任一未见视图提升至50%（仅当前分数的界，非架构上限）。训练712854 dead/exit0；统计727985 dead，tmux退出码空，但/proc为Z且exit_code=0，已核验真实正常结束，不能当live或重启原批。最终核验6740 bytes，SHA 0071a30c1af9e8d557853270a62ba868d60c18f7f8804a0de3ff6042fb2a5707。48原模型/6数据stat/12源码保持，runtime仍EE。下一步先核对容量旧记录并登记保持GCN/GAT+GRU的单项容量控制，当前没有新候选登记或开训；根因具体机制/合理改善仍未完成。

**B4概率尺度检查已完成（46.13）：**源码01a9400通过4服务器检查，9视图上界又用50位Decimal独立重算通过，无阈值扫描/NPZ/forward。在整体P≥.80且允许全部ongoing与起点正确的有利假设下，单一全局阈值/保持排序的单调校准，B4末轮heldout最多可能25/144（17.36%）、原validation23/145（15.86%）；三轮都排除50%召回。不是可达到的阈值结果，更不是架构上限；说明当前排序不够好，不能只归咎.70概率尺度。B5最新38轮，两原pane live；B5完成后可对其JSON运行已部署解析脚本一次，4服务器测试已通过不重跑。
