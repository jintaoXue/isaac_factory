...
2026-09-12 上下文四次已完成，未稳定改善 upcoming；单权重候选正在原目录运行。
本地新增显式 `TRAIN_VARIANT=three_class` 待对照：保持骨干与评估，事件头输出
无事件/ongoing/upcoming，交叉熵监督只使用训练标签，事件概率取两类预测概率之和。
标准 binary 头保留作为有效消融对照，不增加旧数据契约兼容。该候选未部署或训练，
不得将实现/测试通过写成指标提升；先等正在执行的四次加权实验全部结束。
归档诊断可显式传入 `--checkpoint <archive.zip> --archive_member best.pt`，校验哈希后
内存加载，避免为了诊断恢复或覆盖模型文件。相关测试现为 149 项及 12 个子测试通过。
