...
| **评测**（E0 及所有 E* 横比） | **43–52**（10 个，每 seed **2** 局，共 20） | 与训练主 seed 42 错开；`HC_TEST_TIMES` 默认 2 |
| **教师采库** | 可用 **42**（分布对齐）或 42+其它 | **不要**用 43–52 整段扫满当库 |

## E0 运行入口

`bash run_2026_journal_experiments.sh E0 cuda:0`（先加 `--dry-run` 可预览）。模型默认  
`logs/rl_games/HcFactory/hier_2026-08-27_23-17-41/nn/`（可用 `HC_E0_LOAD_DIR` 覆盖），须含 6 个非空 `*_step_1290000.pth`。

固定 N=10、K=10、每步最多 10 次 C/D dispatch、实际 T=40000、epsilon=0；**评测 seed 43–52** 各 2 局，共 20 局。后续 E1–E5 评测沿用该协议，不直接混用历史 5 seeds（42–46）结果。内部 `t_max_anchor=64000` 经 N10/16 换算为 40000，不能直接改成 40000。
...
- **确认实验**：主跑训练 S42；额外训练种子作方差时再开 S；**评测固定 43–52×2**；配对差值与置信区间；验证集选配置，测试集不参与选择。单个 T0 起点结论限于该起点，预训练和采库成本单列。
