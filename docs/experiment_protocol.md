...
| **信用缩放 / B-score** | E4 / E6 全开；`.5` 只开缩放     | **学习**：奖励尺度 / B 探索率   |
| **自回归（AR）**         | E5 / E6；消融名 `*-no-ar`    | **行为**：分层 ε + 步内候选采样（评测仍贪心） |

ORU 与 guide 都用教师，但一个改 **batch**，一个改 **动作**；用 `E3` vs `E3-no-oru` vs `E1` 拆开看。AR 只改在线选动作，不改写 replay。
...
| `credit_scale_A/B`    | `1.0`           | **仅当** `hierarchical_credit=true` 时生效；E4 / E6 / E*.5 显式设 `2.0/1.5` |
| `b_score_rl`          | `False`         | 与 H **正交**；`--hierarchical_credit` **不会**自动打开它                   |
| `teacher_explore`     | `False`         | E3+ / E5 / E6 由 journal 打开                                       |
| `oru`                 | `False`         | E2+ 主线默认开；`*-no-oru` **强制关**，且不要求 catalog                        |
| `autoregressive`      | `False`         | E5 / E6 打开；分层 ε（`ar_eps_scale_*`）+ 步内候选采样（`ar_n_candidates`）   |


**启动日志**必含：`hier_credit=… (A=… B=…) b_score_rl=… teacher_explore=… oru=… ar=…`。  
对已知 `algo_variant`，`HierarchicalTPA` 会按表 **断言** 有效开关；不一致直接 `RuntimeError`。


| 编号            | H   | b_score | A/B   | teacher_explore | oru   | AR    |
| ------------- | --- | ------- | ----- | --------------- | ----- | ----- |
| E1            | 关   | 关       | 1/1   | 关               | 关     | 关     |
| E1.5          | 开   | 关       | 2/1.5 | 关               | 关     | 关     |
| E2            | 关   | 关       | 1/1   | 关               | 开     | 关     |
| E2.5          | 开   | 关       | 2/1.5 | 关               | 开     | 关     |
| E3            | 关   | 关       | 1/1   | 开               | 开     | 关     |
| **E3-no-oru** | 关   | 关       | 1/1   | **开**           | **关** | 关     |
| E3.5          | 开   | 关       | 2/1.5 | 开               | 开     | 关     |
| E4            | 开   | 开       | 2/1.5 | 开               | 开     | 关     |
| **E4-no-oru** | 开   | 开       | 2/1.5 | 开               | **关** | 关     |
| **E5**        | 关   | 关       | 1/1   | 开               | 开     | **开** |
| **E6**        | 开   | 开       | 2/1.5 | 开               | 开     | **开** |
...
```bash
./run_2026_journal_experiments.sh E5 cuda:0
./run_2026_journal_experiments.sh E6 cuda:0
```
...
- **分层 ε**：`ε_layer = ε_base × ar_eps_scale_{A,B,C,D}`（默认 A/B=0.5，C/D=1.0）。与 `b_score_rl` 的 B×0.5 **统一**：开 AR 时只用 `ar_eps_scale_B`，不再额外再乘 0.5。**注：** E5/E6 默认仍开 `teacher_explore`，此时多数步走 exploit（ε_act=0），分层 ε 主要在无 TE / 标准 ε-greedy 路径体现；开 TE 时 AR 的主效果是步内候选采样。
- **步内候选采样**：贪心路径上按 softmax(Q/τ) 无放回抽 `ar_n_candidates`（默认 4）个合法动作，再取其中 Q 最大者；记录 `MetricTrain/22_ar_reselect_rate`（相对全局 argmax 的改选率）。
- **评测**：强制纯贪心（关闭候选采样），保证 seeds 43–52 可复现。
- **不做**：Scheduled sampling / TF（会改写 replay 上游动作却沿用旧转移，协议禁止）。

旧方案曾把「E4+自回归」叫作 E5；现为 **E5=只 AR，E6=AR+H**。
## 1. 命名规则与主实验表
...
**先跑 E0–E4，再跑 E5，最后 E6。** E1 是公平微调基线；**E6 是候选完整方法**。相邻/消融比较：E3-no-oru vs E3 看 ORU，E4 vs E3 看层级学习，E5 vs E3 看自回归，E6 vs E5 / E4 看组合。
...
| `ar`    | 自回归决策改进               | `autoregressive=true`（分层 ε + 候选采样） |
...
- **自回归增强（旧 A）**：已实现分层 epsilon（`ar_eps_scale_*`）与步内合法候选采样（`ar_n_candidates`，Q 评分）。**不**做 replay 改写 / SS / TF。评测关闭候选采样。入口：`E5` / `E6`。
