...
#   CLEAN_DERIVED=1   # remove each run's shared_bn_agg_v1 before rebuilding
#   SEED=42
...
INPUT_WINDOWS="${INPUT_WINDOWS:-12}"
HORIZON="${HORIZON:-180}"
SCORE_THRESHOLD="${SCORE_THRESHOLD:-0.55}"
MIN_EVENT_WINDOWS="${MIN_EVENT_WINDOWS:-1}"
...
printf '  %s\n' "${RUN_DIRS[@]}"
echo "Aggregation: window=${WINDOW_SIZE}s history=${INPUT_WINDOWS} horizon=${HORIZON}s threshold=${SCORE_THRESHOLD}"

if [ "$CLEAN_DERIVED" = "1" ]; then
    for run_dir in "${RUN_DIRS[@]}"; do
        derived_dir="${run_dir}/shared_bn_agg_v1"
