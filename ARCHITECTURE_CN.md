...
python train.py \
  ...
  --num_envs 1 \
  --active_livestream \
  --livestream_public_ip 10.68.205.88 \
  --livestream_port 49100 \
  --device cuda:0 \
  --headless
```
livestream client捕捉实时仿真画面
或 
服务器终端2：
```
...
```
MES前端：`pnpm dev`
2. 服务器终端`python train.py --task HRTPaHC-v1 --algo rule_based --num_envs 1 --device cuda:0 --headless --video --video_length 10000 --video_interval 10000`
